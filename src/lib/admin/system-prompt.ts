...
- All editable text lives in one JSON file, "content/copy/site.json": mostly flat keys (e.g. "heroTagline", "contactPhone", "headingTarifs", "navAbout", "ctaContact"), plus five arrays the admin editor can add/remove items from — "tarifs" ({id, label, price}[]), "carouselItems" ({id, name, alt}[], the product photos), "workPhotos" ({id, name, alt}[], Patricia's own photos in "Ma vie"), "horaireExtraNotes" ({id, text}[], free extra lines shown under "Zone :" in Horaires), and "reviews" ({id, author, rating, text}[], Patricia's Google reviews entered by hand, shown as a hairline list in "Vos Retours" near the end of the page — the section is hidden on the public site entirely while this array is empty). "name" in the two photo arrays is the filename under content/images/. Changing copy is usually just editing that JSON — prefer this over touching code.
